...
const AdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return (
        <div className="flex min-h-screen w-full bg-background-light dark:bg-background-dark font-sans text-gray-900 dark:text-gray-100 transition-colors duration-200">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-64 bg-black border-r border-[#1F1F1F] transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:flex md:flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                    } overflow-y-auto`}
            >
                <div className="flex h-full flex-col justify-between p-4">
                    <div className="flex flex-col gap-8">
                        ...
                        <div className="flex items-center justify-center px-3 py-4">
                            <div className="w-24">
                                <Link to="/area-administrativa/usuarios">
                                    <img src="/logo.png" alt="InflueTech Admin" className="w-full h-full object-contain" />
                                </Link>
                            </div>
                            <span className="ml-2 text-[10px] font-bold text-neon-green bg-neon-green/10 px-2 py-0.5 rounded border border-neon-green/20 tracking-wider">MASTER</span>
                        </div>
            ...
            {/* Main Content */}
            <main className="flex-1 w-full md:w-auto flex flex-col min-w-0 overflow-hidden relative bg-black">
                {/* Top Header */}
                <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#1F1F1F] bg-black/90 backdrop-blur-sm px-4 md:px-8">
                    <div className="flex items-center gap-4">
                        <button
                        ...
                        <h1 className="text-lg font-bold text-white hidden md:block tracking-wide">
                            Área Administrativa
                        </h1>
                    </div>

                    <div className="flex items-center gap-3">
                        {/* Theme Toggle - Hidden in Admin (Always Dark) */}
                        {/* 
                        <button
                            onClick={() => setIsDarkMode(!isDarkMode)}
